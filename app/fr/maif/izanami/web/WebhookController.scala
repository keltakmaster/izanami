...
class WebhookController(
    ...
    implicit request =>
      {
        LightWebhook.lightWebhookRead.reads(request.body) match {
          case JsSuccess(webhook, path) => {
            webhook.bodyTemplate.map(template => {
              Try{
                handlebars.compileInline(template)
                webhook
              }.toEither
            }).getOrElse(Right(webhook)).fold(ex => {
  ...
  def updateWebhook(tenant: String, id: String): Action[JsValue] =
    webhookAuthAction(tenant = tenant, webhook = id, minimumLevel = RightLevels.Write).async(parse.json) { implicit request =>
      {
        (for(
          uuid <- Try{UUID.fromString(id)}.toOption;
          webhook <- LightWebhook.lightWebhookRead.reads(request.body).asOpt
        ) yield {
          env.datastores.webhook.updateWebhook(tenant, uuid, webhook)
            .map {
